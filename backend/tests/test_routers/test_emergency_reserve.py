...
def test_create_emergency_reserve(client, auth_headers):
...
def test_create_emergency_reserve_uniqueness(client, auth_headers):
...
def test_create_regular_class_still_works(client, auth_headers):
...
def test_update_emergency_reserve_keeps_target_weight_zero(client, auth_headers):
...
def test_update_promote_to_reserve_uniqueness(client, auth_headers):
...
def test_delete_and_recreate_reserve(client, auth_headers):
    ...
    # Delete
    del_resp = client.delete(f"/api/asset-classes/{ac_id}", headers=auth_headers)
...
def test_reserve_returned_in_list(client, auth_headers):
    ...
    resp = client.get("/api/asset-classes", headers=auth_headers)
    assert resp.status_code == 200
    classes = resp.json()
